...
echo "----------------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        export my_registry_name=$(oc registry info)
...
echo "---------------------------------------------------------------------------------------------------------------------------"
        ...
        export local_registry=$(oc registry info --internal)
        oc create route reencrypt -n openshift-image-registry --service=image-registry --port 5000
        ...
        then
            while [[ ! $imageExists =~ "Image is up to date" ]]
            do
 ...
 oc create clusterrolebinding icamklust-binding_default --clusterrole=cluster-admin \
          --serviceaccount=multicluster-endpoint:k8monitor -n multicluster-endpoint
