...
// IMInstallSpec defines the desired state of IMInstall
type IMInstallSpec struct {
	// Important: Run "operator-sdk generate k8s" to regenerate code after modifying this file

	// Opt-in license acceptance is required to create resources
	License License `json:"license,omitempty"`

	// Application name used for deployed objects (default: iminstall)
	// +optional
...
// IMInstallStatus defines the observed state of IMInstall
type IMInstallStatus struct {
...
// +kubebuilder:resource:path=iminstalls,scope=Namespaced
type IMInstall struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   IMInstallSpec   `json:"spec,omitempty"`
	Status IMInstallStatus `json:"status,omitempty"`
}

// +k8s:deepcopy-gen:interfaces=k8s.io/apimachinery/pkg/runtime.Object

// IMInstallList contains a list of IMInstall
type IMInstallList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []IMInstall `json:"items"`
}

func init() {
	SchemeBuilder.Register(&IMInstall{}, &IMInstallList{})
}

func (m *IMInstall) Initialize() {
	spec := &m.Spec

	if spec.AppName == "" {
		spec.AppName = "iminstall"
	}
...
func (m *IMInstall) Validate() error {
	spec := m.Spec
	errs := []string{}
	...
	if len(errs) > 0 {
		err := fmt.Sprintf("validation failed for IMInstall object: %s", strings.Join(errs, ", "))
