...
import (
	...
	tlstools "github.ibm.com/IBMPrivateCloud/ibm-infra-management-install-operator/pkg/helpers/tlstools"
...
func NewIngress(cr *iminstallv1alpha1.IMInstall) *extensionsv1beta1.Ingress {
...
func NewHttpdConfigMap(cr *iminstallv1alpha1.IMInstall) *corev1.ConfigMap {
	labels := map[string]string{
		"app": cr.Spec.AppName,
	}
...
func NewHttpdAuthConfigMap(cr *iminstallv1alpha1.IMInstall) *corev1.ConfigMap {
	labels := map[string]string{
		"app": cr.Spec.AppName,
	}
	data := map[string]string{
...
func configureHttpdAuth(spec *iminstallv1alpha1.IMInstallSpec, podSpec *corev1.PodSpec) {
...
func getHttpdImage(namespace, tag string, privileged bool) string {
	var image string
	if privileged {
		image = "httpd-init"
	} else {
...
func initializeHttpdContainer(spec *iminstallv1alpha1.IMInstallSpec, privileged bool, c *corev1.Container) error {
	c.Name = "httpd"
	httpdImage := os.Getenv("HTTPD_IMAGE")
	if httpdImage == "" {
		httpdImage = getHttpdImage(spec.HttpdImageNamespace, spec.HttpdImageTag, privileged)
	}
	c.Image = httpdImage
	if privileged {
...
func NewHttpdDeployment(cr *iminstallv1alpha1.IMInstall) (*appsv1.Deployment, error) {
...
func NewUIService(cr *iminstallv1alpha1.IMInstall) *corev1.Service {
	labels := map[string]string{
		"app": cr.Spec.AppName,
	}
...
func NewWebService(cr *iminstallv1alpha1.IMInstall) *corev1.Service {
	labels := map[string]string{
		"app": cr.Spec.AppName,
	}
	selector := map[string]string{
...
func NewRemoteConsoleService(cr *iminstallv1alpha1.IMInstall) *corev1.Service {
	labels := map[string]string{
		"app": cr.Spec.AppName,
	}
	selector := map[string]string{
...
func NewHttpdService(cr *iminstallv1alpha1.IMInstall) *corev1.Service {
	labels := map[string]string{
		"app": cr.Spec.AppName,
	}
...
func NewHttpdDbusAPIService(cr *iminstallv1alpha1.IMInstall) *corev1.Service {
	labels := map[string]string{
		"app": cr.Spec.AppName,
	}
...
func TLSSecret(cr *iminstallv1alpha1.IMInstall) (*corev1.Secret, error) {
	labels := map[string]string{
		"app": cr.Spec.AppName,
	}
...
func tlsSecretName(cr *iminstallv1alpha1.IMInstall) string {
